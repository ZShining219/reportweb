...
- 可折叠树形节点列表导航；
- 聚焦当前节点的亲属走廊节点图导航；
- 节点图支持高扇出亲属窗口、省略标记和当前节点居中保护；
- 左右侧栏以边缘按钮独立展开，不使用遮罩，不影响主板呈现；
- 右侧 inspector 与 revision 面板模块化渲染；
- 灵动岛项目入口与轻量项目管理工作台，可在同一布局中浏览、选中并进入已有汇报项目；
- 全局 UI token、基础 UI class 和模块 demo 基座；
- Node 内置测试覆盖 report patch 合并、节点图、折叠树、diff、API、编辑反馈、侧栏抽屉、画布平移和面板逻辑。
...
- 项目管理画面复用中间舞台、左侧栏、右侧栏和灵动岛，不跳到独立页面区域；
- 中间舞台显示已经由 Codex 生成并落盘的汇报项目看板；
- 点击中间舞台的项目卡片直接进入具体汇报；
- 点击左侧项目列表项选中项目，并同步更新右侧详情；
- 点击项目卡片内或右侧详情中的“进入汇报”也可进入具体汇报；
- Web 端第一期不提供新建项目入口。
...
- 底部节点图只呈现当前选中节点附近的亲属关系，不等同于左侧可折叠完整树；
- 当前节点固定在视觉中心，父节点、当前节点和主子节点默认落在同一横轴；
- 横向按五列组织亲属走廊：远祖、父层、当前层、子层、边缘后代层；
- 当前层和边缘层会优先保留靠近当前轴的近邻窗口，过远节点使用 `+N` 省略标记提示；
- 高扇出数据下，节点图会限制可见亲属窗口，保证节点和省略标记仍留在导航画布范围内。
...
- 左右侧栏抽屉状态目前由 `public/app.js` 装配层管理，视觉规则在 `public/styles.css` 与 `public/ui/tokens.css` 中定义。
...
- `public/nodeContext.js`：生成聚焦当前节点的亲属走廊布局，包含五列布局、近邻窗口、同列间距归一、省略节点统计和高扇出 bounds 保护。
- `public/nodeNavigator.js`：将节点图模型渲染为底部 SVG 导航和可点击命中层，包含 `+N` 省略标记提示。
- `tests/node-context.test.js`、`tests/node-navigator.test.js`：覆盖亲属布局、第五列扩展、同列间距、省略标记和高扇出压力场景。
...
2. 汇报项目管理：补齐项目搜索、筛选、排序、丰富元数据和 Codex 生产流程状态展示。
